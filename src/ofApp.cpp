...
void ofApp::setup() {
	...
	if (!diffuseShader.loadCompute("diffuseShader.glsl")) {
		ofLogError("Shader Program") << "Could not load diffuse shader";
	}
	else {
	...
	trailMapTexture.allocate(windowWidth, windowHeight, GL_RGBA32F); 
	trailMapTexture.bindAsImage(1, GL_READ_WRITE);

	// Initialize uniform variables
	// These variables are very picky and slight changes can completely alter the output
	moveSpeed = 2.6f; // The speed at which slimes move
	sensorSize = 1;
